...
class ModbusClient(IModbusGateway):
    ...
    def __init__(self, port, slave_id, baudrate=9600, parity=serial.PARITY_EVEN, stopbits=1, timeout=0.5):
        ...
        self.logger.debug("Inicializando ModbusClient con parámetros: port=%s, slave_id=%s, baudrate=%s, parity=%s, stopbits=%s, timeout=%s", port, slave_id, baudrate, parity, stopbits, timeout)
        try:
            self.instrument = minimalmodbus.Instrument(port, slave_id)
            self.logger.debug("Instrumento minimalmodbus.Instrument creado correctamente.")
    ...
    def read_register(self, registeraddress, number_of_decimals=1, functioncode=3, signed=False):
        ...
        self.logger.debug(
            "Intentando leer registro: address=%s, number_of_decimals=%s, functioncode=%s, signed=%s",
        ...
        try:
            ...
            self.logger.info("Lectura exitosa del registro %s: %s", registeraddress, value)
            self.logger.debug("Valor leído (debug): %s", value)
            if value is None:
                self.logger.warning("El valor leído del registro %s es None", registeraddress)
            return value
        except (serial.SerialException, IOError) as e:
            ...
            self.logger.critical("Excepción crítica leyendo registro %s: %s", registeraddress, ex)
            raise
